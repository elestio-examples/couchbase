...
docker-compose exec couchbase bash -c "couchbase-cli cluster-init -c 127.0.0.1 --cluster-username ${COUCHBASE_ADMINISTRATOR_USERNAME} \
 --cluster-password ${ADMIN_PASSWORD} --cluster-name couchbase --services data --cluster-ramsize 4096"

 docker-compose exec couchbase bash -c "couchbase-cli setting-alert -c 127.0.0.1 --username ${COUCHBASE_ADMINISTRATOR_USERNAME} \
